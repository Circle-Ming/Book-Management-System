...
<html>
    ...
    <body>
        ...
        <form method = "post">
                ...
                <button name="login"  type="submit" class = "btn">登录</button>
                <button name="register"  type="submit" class = "btn">注册</button>
                <button name="Admin_login"  type="submit" class = "btn">管理员界面</button>
        </form>
        ...
        <?php 
            ...
            {   
                ...
                if (isset($_POST['login'])){
                    $query = "select * from userInfo where name = '{$_POST['name']}' and pw = '{$_POST['pw']}'";
                    $result = mysqli_query($conn, $query);
                    $rcount = mysqli_num_rows($result);
                    if ($rcount ==1){     	
                        echo "<script>alert('登录成功！');parent.location.href='1.php';</script>";
                        return;
                    }
                    else{
                    echo "<script>alert('用户或密码错误！');parent.location.href='login.php';</script>";
                    return;
                    }
                }
            }
